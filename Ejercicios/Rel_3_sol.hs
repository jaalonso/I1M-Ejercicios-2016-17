...
-- irecasmat marvilmor migandben alebergon fatvilpiz
suma :: Integer -> Integer
suma n = sum [x | x <- [1..n]]
...
-- abrdelrod juanarcon manpende paocabper alelobcan migandben
linea :: Integer -> [Integer]
...
-- guache carruirui3 lucgamgal juanarcon paocabper juamorrom1 marvilmor
perfectos2 :: Int -> [Int]
...
-- carmengar erisancha pabmorgar alvalvdom1 ivaruicam blaruiher fatvilpiz
-- abrdelrod manvazbar1 manpende alelobcan marvilmor
numeroAbundante :: Int -> Bool
...
-- guache carmengar manpende isrbelnun
primerAbundanteImpar :: Int
...
-- alvalvdom1 lucgamgal fracruzam juanarcon manvazbar1 ivaruicam
-- juamorrom1 irecasmat migandben fatvilpiz
euler1c :: Int -> Int
...
-- alelobcan isrbelnun migandben alebergon javperlag
circulo :: Int -> Int
...
-- isrbelnun marvilmor alebergon fatvilpiz javperlag
aproxE :: Double -> [Double]
...
-- carmengar carruirui3 erisancha josllagam abrdelrod blaruiher
-- manvazbar1 
errorAproxE :: Double -> Double
...
-- paocabper alebergon fatvilpiz javperlag
aproxLimSeno :: Double -> [Double]
...
-- juamorrom1 silgongal isrbelnun alebergon fatvilpiz migandben 
-- javperlag
pitagoricas :: Int -> [(Int,Int,Int)] 
...
-- alvalvdom1 blaruiher rubvilval paocabper isrbelnun marvilmor
numeroDePares :: (Int,Int,Int) -> Int
...
numeroDePares3 (x,y,z) = 3 - x`mod`2- y`mod`2 - z`mod`2
...
-- carruirui3 erisancha silgongal migandben
...
-- Hemos de probar que en todas las ternas pitagóricas hay un número
-- impar de elementos pares. Lo haremos mediante REDUCCIÓN AL ABSURDO: 
-- 
-- CASO 1: Ningún par
-- 
-- imp^2+imp^2=imp^2
-- imp+imp=imp
-- CONTRADICCION: La suma de dos números impares siempre es un número par.
-- 
-- CASO 2: 2 elementos pares
-- 
-- a)
-- imp^2+par^2=par^2
-- imp+par=par
-- CONTRADICCION: La suma de un número par y otro impar siempre es un
-- número impar. 
-- 
-- b)
-- par^2+par^2=imp^2
-- par+par=imp
-- CONTRADICCION: La suma de dos números pares siempre es un número par.
...
-- ivaruicam fatvilpiz migandben javperlag
...
-- alvalvdom1 rubvilval juanarcon abrdelrod manpende juamorrom1 javperlag
-- silgongal isrbelnun ivaruicam paocabper alebergon fatvilpiz migandben
productoEscalar2 :: [Int] -> [Int] -> Int 
productoEscalar2 xs ys = sum [x*y | (x,y) <- zip xs ys ]

-- carruirui3 erisancha blaruiher manvazbar1 marvilmor
...
-- juamorrom1 silgongal isrbelnun ivaruicam alebergon javperlag
...
-- guache juanarcon abrdelrod manpende fracruzam silgongal ivaruicam
densa :: [Int] -> [(Int,Int)]
densa xs = 
...
-- carruirui3 lucgamgal rubvilval javperlag
densa3 :: [Int] -> [(Int, Int)]
...
-- blaruiher abrdelrod manpende manvazbar1 fatvilpiz migandben javperlag
musicos :: [(String,String,Int,Int)] -> [String]
...
-- manvazbar1 silgongal isrbelnun marvilmor ivaruicam alebergon migandben
-- javperlag
seleccion :: [(String,String,Int,Int)] -> String -> [String]
...
-- guache carruirui3 manvermor lucgamgal pabmorgar alvalvdom1 erisancha
-- juanarcon blaruiher rubvilval abrdelrod fracruzam josllagam javperlag
...
-- manvazbar1 silgongal marvilmor ivaruicam alebergon migandben javperlag
vivas :: [(String,String,Int,Int)] -> Int -> [String]
...
vivas3 bd a = [nom | (nom,_,nac,mue) <- bd, mue >= a, nac <= a]
